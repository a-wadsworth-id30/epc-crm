"use client";

import dynamic from "next/dynamic";
import type { SalesPipelineStageManagerItem } from "@/components/crm-boilerplate/SalesPipelineStageManager";
import type { SalesKanbanSettings } from "@/lib/sales/kanban-settings";

export type { SalesPipelineStageManagerItem };

type SalesPipelineStageManagerProps = {
  kanbanSettings: SalesKanbanSettings;
  nextSortOrder: number;
  stages: SalesPipelineStageManagerItem[];
};

function SalesPipelineStageManagerLoading() {
  return (
    <div>
      <div className="grid gap-4 md:grid-cols-4">
        {["stages", "active", "buckets", "linked-sales"].map((item) => (
          <div
            key={item}
            className="rounded-2xl border border-gray-200 bg-white p-5 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]"
          >
            <div className="h-3 w-20 rounded bg-gray-100 dark:bg-white/[0.08]" />
            <div className="mt-3 h-7 w-16 rounded bg-gray-50 dark:bg-white/[0.05]" />
          </div>
        ))}
      </div>

      <section className="mt-6 overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
        <div className="border-b border-gray-200 p-5 dark:border-gray-800">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <div className="h-5 w-40 rounded bg-gray-100 dark:bg-white/[0.08]" />
              <div className="mt-3 h-4 w-full max-w-md rounded bg-gray-50 dark:bg-white/[0.05]" />
            </div>
            <div className="h-9 w-28 rounded-lg bg-gray-100 dark:bg-white/[0.08]" />
          </div>
        </div>

        <div className="divide-y divide-gray-100 lg:hidden dark:divide-gray-800">
          {["one", "two", "three", "four"].map((item) => (
            <div key={item} className="space-y-3 px-5 py-4">
              <div className="flex items-start gap-3">
                <div className="mt-1 h-3 w-3 shrink-0 rounded-full bg-gray-100 dark:bg-white/[0.08]" />
                <div className="min-w-0 flex-1">
                  <div className="h-4 w-36 max-w-full rounded bg-gray-100 dark:bg-white/[0.08]" />
                  <div className="mt-2 h-3 w-24 max-w-full rounded bg-gray-50 dark:bg-white/[0.05]" />
                  <div className="mt-2 h-3 w-48 max-w-full rounded bg-gray-50 dark:bg-white/[0.05]" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="h-6 rounded-full bg-gray-50 dark:bg-white/[0.05]" />
                <div className="h-4 rounded bg-gray-50 dark:bg-white/[0.05]" />
                <div className="h-6 rounded-full bg-gray-50 dark:bg-white/[0.05]" />
                <div className="h-4 rounded bg-gray-50 dark:bg-white/[0.05]" />
              </div>
            </div>
          ))}
        </div>
        <div className="hidden max-w-full min-w-0 overflow-x-auto lg:block">
          <div className="min-w-[860px] divide-y divide-gray-100 dark:divide-gray-800">
            <div className="grid grid-cols-[0.7fr_1.7fr_1fr_0.9fr_1fr_1fr_0.7fr] gap-4 bg-gray-50 px-5 py-3 dark:bg-white/[0.02]">
              {[
                "Order",
                "Stage",
                "Bucket",
                "Probability",
                "Status",
                "Usage",
                "Actions",
              ].map((item) => (
                <div
                  key={item}
                  className="h-3 w-20 rounded bg-gray-100 dark:bg-white/[0.08]"
                >
                  <span className="sr-only">{item}</span>
                </div>
              ))}
            </div>
            {["one", "two", "three", "four"].map((item) => (
              <div
                key={item}
                className="grid grid-cols-[0.7fr_1.7fr_1fr_0.9fr_1fr_1fr_0.7fr] gap-4 px-5 py-4"
              >
                <div className="h-4 w-10 rounded bg-gray-50 dark:bg-white/[0.05]" />
                <div className="flex items-start gap-3">
                  <div className="mt-1 h-3 w-3 rounded-full bg-gray-100 dark:bg-white/[0.08]" />
                  <div>
                    <div className="h-4 w-36 rounded bg-gray-100 dark:bg-white/[0.08]" />
                    <div className="mt-2 h-3 w-24 rounded bg-gray-50 dark:bg-white/[0.05]" />
                    <div className="mt-2 h-3 w-48 rounded bg-gray-50 dark:bg-white/[0.05]" />
                  </div>
                </div>
                <div className="h-6 w-24 rounded-full bg-gray-50 dark:bg-white/[0.05]" />
                <div className="h-4 w-12 rounded bg-gray-50 dark:bg-white/[0.05]" />
                <div>
                  <div className="h-6 w-20 rounded-full bg-gray-50 dark:bg-white/[0.05]" />
                  <div className="mt-2 h-3 w-24 rounded bg-gray-50 dark:bg-white/[0.05]" />
                </div>
                <div>
                  <div className="h-4 w-16 rounded bg-gray-50 dark:bg-white/[0.05]" />
                  <div className="mt-2 h-3 w-28 rounded bg-gray-50 dark:bg-white/[0.05]" />
                </div>
                <div className="ml-auto h-9 w-9 rounded-lg bg-gray-50 dark:bg-white/[0.05]" />
              </div>
            ))}
          </div>
        </div>
      </section>
    </div>
  );
}

const SalesPipelineStageManager = dynamic<SalesPipelineStageManagerProps>(
  () => import("@/components/crm-boilerplate/SalesPipelineStageManager"),
  {
    loading: SalesPipelineStageManagerLoading,
    ssr: false,
  },
);

export default function LazySalesPipelineStageManager(
  props: SalesPipelineStageManagerProps,
) {
  return <SalesPipelineStageManager {...props} />;
}
