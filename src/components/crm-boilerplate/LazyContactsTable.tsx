...
import type { ContactCategoryValue } from "@/lib/contacts/categories";
  ...
  return (
    <div className="space-y-5">
      <section className="space-y-4">
        <div className="h-10 w-full max-w-lg rounded-lg bg-gray-50 dark:bg-white/[0.05]" />
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {["Consumer", "Trade", "Installer", "Company"].map((item) => (
            <div
              key={item}
              className="h-40 rounded-lg border border-gray-100 bg-white shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]"
            />
          ))}
        </div>
      </section>
      <section className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
        ...
        <div className="hidden max-w-full min-w-0 overflow-x-auto lg:block">
          <div className="min-w-[1040px] divide-y divide-gray-100 dark:divide-gray-800">
            <div className="grid grid-cols-[1.2fr_1fr_0.8fr_0.9fr_0.9fr_1.2fr_1fr_1fr_0.6fr] gap-4 bg-gray-50 px-5 py-3 dark:bg-white/[0.02]">
              {[
                "Name",
                "Company",
                "Category",
            ...
            {["one", "two", "three", "four"].map((item) => (
              <div
                key={item}
                className="grid grid-cols-[1.2fr_1fr_0.8fr_0.9fr_0.9fr_1.2fr_1fr_1fr_0.6fr] gap-4 px-5 py-4"
              >
