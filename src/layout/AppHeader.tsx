...
import LazyNotificationDropdown from "@/components/header/LazyNotificationDropdown";
import UserDropdown from "@/components/header/UserDropdown";
import AppLogo from "@/components/crm-boilerplate/AppLogo";
import QuickCreateMenu from "@/components/crm-boilerplate/QuickCreateMenu";
...
import HeaderSearch from "@/layout/LazyHeaderSearch";
import Link from "next/link";
import { useState } from "react";

const SidekickAssistant = dynamic(
  () => import("@/components/crm-boilerplate/SidekickAssistant"),
  {
    loading: () => null,
    ssr: false,
  },
        ...
        <div className="flex items-center justify-between w-full gap-2 px-3 py-3 border-b border-gray-200 dark:border-gray-800 sm:gap-4 xl:justify-normal xl:border-b-0 xl:px-0 xl:py-4">
          ...
          <div className="hidden items-center gap-2 xl:flex">
            <HeaderSearch
              ...
              moduleToggles={moduleToggles}
            />
            <QuickCreateMenu companiesEnabled={companiesEnabled} />
          </div>
        </div>
        <div
          className={`${
            isApplicationMenuOpen ? "flex" : "hidden"
          } w-full flex-col gap-3 px-4 py-4 shadow-theme-md xl:flex xl:flex-row xl:items-center xl:justify-end xl:px-0 xl:shadow-none`}
        >
          ...
          <div className="flex w-full items-center justify-between gap-3 xl:w-auto xl:justify-end">
            <div className="flex items-center gap-2 2xsm:gap-3">
              ...
              <LazyNotificationDropdown notifications={notifications} />
              {/* <!-- Notification Menu Area --> */}
            </div>
            {/* <!-- User Area --> */}
            <UserDropdown currentUser={currentUser} />
            {moduleToggles.ai ? <SidekickAssistant /> : null}
          </div>
        </div>
      </div>
    </header>
