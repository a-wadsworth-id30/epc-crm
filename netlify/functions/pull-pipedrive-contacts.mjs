...
export default async function handler() {
  if (!booleanEnv("PIPEDRIVE_CONTACT_IMPORT_CRON_ENABLED")) {
    console.info("Scheduled Pipedrive contact import skipped", {
      reason: "disabled",
    });
    ...
    return Response.json({
      ok: true,
      skipped: true,
      message: "Scheduled Pipedrive contact import is disabled.",
    });
  }
  ...
  if (!secret) {
    console.error("Scheduled Pipedrive contact import failed", {
    ...
    return Response.json(
      {
        ok: false,
        error:
          "PIPEDRIVE_CONTACT_IMPORT_SECRET, PIPEDRIVE_LEAD_IMPORT_SECRET or CRON_SECRET is required.",
      },
      { status: 500 },
    );
  }
  ...
  const url = new URL("/api/maintenance/pipedrive-contact-import", baseUrl.url);
  url.searchParams.set(
    "dryRun",
    booleanEnv("PIPEDRIVE_CONTACT_IMPORT_CRON_DRY_RUN") ? "1" : "0",
  );
  ...
  const response = await fetch(url, {
    ...
    headers: {
      ...
      "user-agent": "iD30 CRM scheduled Pipedrive contact import",
  ...
  if (response.ok) {
    console.info("Scheduled Pipedrive contact import completed", summary);
  } else {
    console.error("Scheduled Pipedrive contact import failed", summary);
  }
