...
- Settings > Integrations includes Pipedrive as a real CRM data integration.
  ...
  records a CRM-only `webhook-receiver-test` sync log, shows inline
  success/failure feedback in the receiver panel and does not read or write
  Pipedrive records.
...
- Settings > Integrations includes Spruce as a real integration. The CRM stores
  encrypted Spruce webhook/API credentials in `IntegrationConnection.config` or
  reads `SPRUCE_WEBHOOK_SECRET`, `ZAPIER_SPRUCE_WEBHOOK_SECRET` and
  `SPRUCE_API_KEY` from the runtime. The authenticated receiver at
  `/api/webhooks/spruce` supports bearer, basic-auth password,
...
- Desktop Sales bulk selection and sale detail pages include admin-only
  ...
  attribution records, and convert Pipedrive lead/deal links into
  `salesOpportunityDeleted` tombstones so future Pipedrive pull/webhook imports
  skip CRM-deleted opportunities. They must not write to Pipedrive.
...
- Contact detail uses the same postal context in the page header and profile
  ...
  company and role rows with compact contextual icons. Company and role are
  grouped together in the header and profile panel, and are not duplicated in
  the workspace summary strip. The user-facing workspace label follows the
  People section wording.
